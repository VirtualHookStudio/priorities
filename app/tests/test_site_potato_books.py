...
class TestSitePotatoBooks(unittest.TestCase):
    ...
    def test_requester_failure_flow(self):
        ...
        with self.assertLogs(level='ERROR') as log:
            ...
            self.assertRegex(
                log.output[0],
                r"req must be a dict|Missing required key: config|" \
                "config must be a dict|Missing required key: user|user must be a dict|" \
                "Missing required key: user_email"
            )